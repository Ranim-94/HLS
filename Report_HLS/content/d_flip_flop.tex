...
\label{sec:seq_circuit}

$\mathrm{1}^\mathrm{st}$ we start by general definition of sequential circuit, then move to some more concrete example about how sequential circuit behaves through time.

\subsection{Definition}

Sequential circuit uses input from previous states, so they need some system to store or transfer input values. Memory cells are perfect candidates for that.
