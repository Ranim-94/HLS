...
In this project, the `tb` files are served as input for waveform views, in which we givethe input `d` some value, we call the top function, and then capture the output `qn`.
